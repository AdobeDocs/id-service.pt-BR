---
description: Essas configurações permitem que diferentes instâncias do código do Serviço de ID de visitante implementado em um iFrame e na página pai se comuniquem entre si. Foram projetadas para ajudar a resolver problemas com casos de uso específicos onde pode-se ou não controlar a página ou o domínio principal e onde há código do Serviço de ID do visitante sendo carregado no iFrame de um domínio sob seu controle. Eles estão disponíveis no código VisitorAPI.js versão 2.2 ou superior.
...
Essas configurações permitem que diferentes instâncias do código do Serviço de ID de visitante implementado em um iFrame e na página pai se comuniquem entre si. Foram projetadas para ajudar a resolver problemas com casos de uso específicos onde pode-se ou não controlar a página ou o domínio principal e onde há código do Serviço de ID do visitante sendo carregado no iFrame de um domínio sob seu controle. Eles estão disponíveis na versão 2.2 ou superior do código `VisitorAPI.js`.
...
O [!UICONTROL Visitor ID Service] código configurado é semelhante ao deste exemplo.
...
Essas configurações ajudam a resolver o problema de configuração de um cookie do Serviço de ID de visitante e atribuição de uma ID de visitante quando os navegadores bloqueiam cookies de terceiros e se qualquer uma dessas condições se aplicar:

* Você controla ou não a página/domínio pai.
* O código do Serviço de ID de visitante não está instalado na página principal, mas é implementado em um iFrame.

>[!TIP]
>
>Você também pode implementar essas configurações quando estiver disponibilizando vídeo em um iFrame com o [Video Heartbeat](https://experienceleague.adobe.com/docs/media-analytics/using/media-overview.html?lang=pt-BR). O Video Heartbeat precisa de uma ID de serviço de ID de visitante (a MID) para funcionar corretamente.

**Caso de uso 1: o navegador bloqueia cookies de terceiros e o serviço de ID de visitante é implementado no iFrame e na página pai**
...
<table id="table_B479AA96DBE64685A253A6DF98D81B31"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p>Este caso de uso inclui as seguintes condições: </p> <p> 
     <ul id="ul_DC748846585745B0AB74398D82BDA53A"> 
      <li id="li_6E04CF0B6A204B4D8856656B0C9EF2A5">A Empresa A implementa o Serviço de ID de visitante em suas home pages. </li> 
      <li id="li_B53AE0F0C69844E7B6C4D3464C57883B">A Empresa A implementa o Serviço de ID de visitante no iFrame na sua home page. </li> 
      <li id="li_07E0A6D7BEB140E4B9FB6C7B9629B860">A Empresa A é proprietária da página pai e do iFrame e implementou o Serviço de ID de visitante em ambos os locais. </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Dadas essas condições, o Serviço de ID de visitante: </p> <p> 
     <ul id="ul_12356701501E40DFA57903494FFE58F7"> 
      ...
      <li id="li_BA9F42C759E747EAAE14DD3FBB6130A5">Não funciona no iFrame. Isso ocorre porque o navegador vê o iFrame como um domínio de terceiros e impede que o Serviço de ID do visitante defina o cookie AMCV. </li> 
     </ul> </p> </td> 
  </tr> 
  <tr> 
   <td colname="col1"> <p> <b>Solução</b> </p> </td> 
   <td colname="col2"> <p>Modifique a função Visitor.getInstance </span> do Serviço de ID do visitante <span class="codeph"> no iFrame com essas configurações de lista de permissões. Especifique os domínios pai e filho no código. Essas configurações permitem que o código do Serviço de ID de visitante no iFrame verifique o código do Serviço de ID de visitante na página pai de uma ID de visitante. </p> <p>Se o código do Serviço de ID de visitante no iFrame não receber uma página pai de resposta, essas configurações geram uma ID de visitante local. </p> </td> 
  </tr> 
 </tbody> 
</table>

**Caso de uso 2: Solicitação de uma ID de um iFrame incorporado em uma página pai que você não controla ou que não usa o Serviço de ID de Visitante**
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Este caso de uso inclui as seguintes condições: </p> <p> 
     <ul id="ul_356E8FB0B1D14F46A844FE5281967E28"> 
      <li id="li_1285D945361842268B46FB492A3B5AA5">A Empresa A não usa o Serviço de ID de visitante. </li> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Dadas essas condições, o Serviço de ID de visitante: </p> <p> 
     <ul id="ul_A92D90896E5A42C5804AC5CE83E8EB25"> 
      <li id="li_9734EA9C5D9D4F908DE783188C9E5530">Não funciona no iFrame. Isso ocorre porque o navegador vê o iFrame como um domínio de terceiros e impede que o Serviço de ID do visitante defina o cookie AMCV. </li> 
...
* O Serviço de ID de visitante implementado no domínio pai e o domínio iFrame devem usar a mesma ID da organização IMS. Essas configurações de lista branca não funcionarão quando as IDs da organização IMS no pai ou no iFrame forem diferentes.
* Essas configurações só se comunicam com o domínio e os iFrames especificados no código.
* A comunicação entre o iFrame e a página pai segue um formato específico. Se o Serviço de ID de visitante na página pai não receber uma solicitação no formato esperado, esse processo de compartilhamento falhará.

## Métodos de API do visitante suportados {#section-30c6a9f4dcdc4265a1149260b97cc057}

O Serviço de ID de visitante oferece suporte a um conjunto limitado de métodos de API públicos ao implementar essas configurações de lista branca. Os métodos suportados variam de acordo com os cenários de caso de uso descritos acima.
